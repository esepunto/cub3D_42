...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   gerdataro_turn_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ssacrist <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/05 17:06:26 by ssacrist          #+#    #+#             */
/*   Updated: 2021/01/19 19:19:23 by ssacrist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		take_photo(t_cub3d *a)
{
	...
	if ((fd = open("./gerda_taro_bonus.bmp", O_CREAT | O_RDWR, S_IRWXU)) < 0)
		msg_err("I can't create the file.");
	file_size = 54 + (a->fconf.xrendersize * a->fconf.yrendersize * 4);
	write_header(fd, a, file_size);
	write_image(fd, a);
	close(fd);
	ft_printf("File gerda_taro_bonus.bmp has been saved successfully.\n\n");
	close_window(a);
}
